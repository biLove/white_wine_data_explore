...
从上图可以看出，大部分的 citric.acid 含量在0.2~0.5 之间，很难找到 citric.acid 与 quality的相关关系 。
...
3. 探索2个或3个变量的关系时，因为从图中看，变量之间关系并不明确，经常出现不知道怎么描述的情况。
这一块目前的做法时，描述趋势明显的这一部分，并指出哪部分趋势不明显，已经可能造成误差的原因。    
4. 在R的各种方法使用过程中，并不是特别熟悉，故而有时候并不能很快找到最适合的表现方式，这部分需要再多熟悉。   

### 提议    
1. 做探索性数据分析时，很有可能变量之间只有非常微弱的关系，这时候需要耐心寻找这种关系，并通过各种方式，让这种关系合理的呈现出来。
2. 可以先从常识判断哪些变量之间可能存在较大关系，然后再通过GGally库的各种方法来验证想法，并通过各种图形来直观的感受里面存在的关系
